...
-(void)textViewShowView:(UIViewController *)viewController customKeyboardDelegate:(id)delegate
{
    ...
    self.mBackView =[[UIView alloc]initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height-50+50-64, WIDTH_SCREEN, 50)];
    NSLog(@"%p",self.mBackView);
    self.mBackView.backgroundColor =[UIColor colorWithRed:195/255.0 green:235/255.0 blue:250/255.0 alpha:1];
    [self.mViewController.view addSubview:self.mBackView];
    //蓝色条框
    self.mSecondaryBackView =[[UIView alloc]initWithFrame:CGRectMake(10, 10, WIDTH_SCREEN-80, 30)];
    self.mSecondaryBackView.backgroundColor =COLOR_TIANLAN;
    [self.mBackView addSubview:self.mSecondaryBackView];
    
    self.mTextView =[[UITextView alloc]initWithFrame:CGRectMake(1, 1, WIDTH_SCREEN-80-2, 28)];
    ...
    [self.mTalkBtn setTitleColor:COLOR_TIANLAN forState:UIControlStateNormal];
    self.mTalkBtn.titleLabel.font=[UIFont systemFontOfSize:16];
    [self.mTalkBtn addTarget:self action:@selector(forTalk) forControlEvents:UIControlEventTouchUpInside];
    self.mTalkBtn.frame =CGRectMake(ScreenWidth-40-10, 16, 40, 18);
...
{    
    self.isTop =NO;
        [UIView animateWithDuration:0.3 animations:^{
        self.mBackView.frame=CGRectMake(0, HEIGHT_SCREEN-30+50-64, WIDTH_SCREEN, 50);
        self.mHiddeView.alpha =0.0f;
        self.mSecondaryBackView.frame =CGRectMake(10, 10, WIDTH_SCREEN-80,30);
    } completion:^(BOOL finished) {
...
- (void)textDidChanged:(NSNotification *)notif //监听文字改变 换行时要更改输入框的位置
{
    ...
    self.mSecondaryBackView.frame =CGRectMake(10, 10, WIDTH_SCREEN-80, selfHeight-20);
    NSLog(@"文字改变");
}
